...
/*
 * Each dn_queue is followed by scheduler-specific data of size q_datalen.
 * It also contains a block, struct dn_flow == ni which is defined
 * in dn_test.h where we can put additional per flow information.
 *
 * In this case dn_flow contains stats (q_wfi) and a pool
 * of packets to be used for transmissions.
 * The sender can grab any packet whose len is 0 and send it,
 * the scheduler will clear the length when done.
 * For convenience we use an index, tail, to indicate the
 * next buffer to transmit. We can only transmit if mb[tail].len == 0
 */
...
drop(struct cfg_s *c, struct mbuf *m)
{
	...
	ND("q %d id %d current length %d", q->fid, m->flow_id, i);
	if (i < BACKLOG) {
...
default_dequeue(struct dn_sch_inst *si)
{
	struct mq *mq = (struct mq *)si;
	/* this is the default function if no scheduler is provided */
	return mq_dequeue(mq);
}

/*
 * Statistics: on the first enqueue, we record the scheduler state
 * (how many bytes were served in total) and clear the number for
 * this specific queue. The info will be used on dequeue
 * to compute the wfi when the queue drains.
 * We only need the scheduler instance and the queue id.
 */
static void
gnet_stats_enq(struct dn_sch_inst *si, struct dn_queue *_q)
{
	if (_q->ni.length == 1) {
		_q->ni.bytes = 0;
...
/*
 * Statistics: on dequeue we record the number of bytes served,
 * then if this is the last packet we also compute the wfi and
 * take the maximum.
 * We need the schedul
 */
static void
	...
	for (i=0; i < c->loops; i++) {
		...
		if ( (m = c->tosend) ) {
			...
			if (ret) {
				...
				ND("enqueue ok");
				c->pending++;
				gnet_stats_enq(c->si, FI2Q(c, m->flow_id));
			}
...
init(struct cfg_s *c)
{
	...
	c->schk_len = sizeof(struct dn_schk);
	...
	while (ac > 1) {
		...
		} else if (!strcmp(*av, "-shmem")) {
			c->shm_name = strdup(av[1]);
			DX(3, "setting shmem to %s", c->shm_name);
		} else {
...
main(int ac, char *av[])
{
	...
	for (i = 0; i < c.flows; i++) {
		if (c.wfi < FI2Q(&c, i)->ni.q_wfi)
			c.wfi = FI2Q(&c, i)->ni.q_wfi;
	}
...
controller(struct cfg_s *c)
{
	...
	if (c->freelist) {
		...
		c->freelist = c->freelist->m_freelist;
	} else {
...
sched_deq(void *opaque)
{
	...
	if (m) {
	    ...
	    D("--- ouch, cannot operate, pending %d", c->pending);
	}
    }
    return m;
}
